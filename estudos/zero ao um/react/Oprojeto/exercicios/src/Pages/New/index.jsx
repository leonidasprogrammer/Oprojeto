...
const New = () => {
 ...
 async function handleRegister(e){
  ...
  await addDoc(collection(db, 'chamados'),{
   ...
   toast.error('Ocorreu algum erro ao registra um chamado!');
   console.error(error);
 }).finally(() => setloadInserChamados(false));
  }
   ...
   useEffect(() => {
    ...
    async function loadCustomers(){
      ...
      await getDocs(collection(db,'customers')).then((snapshot) => {
        ...
        toast.error('Ocorreu algum erro ao ler dados da empresas!');
        console.error(error);
   ...
   <div>
    ...
    <div className="container">
      <form className="form-profile" onSubmit={handleRegister} >
        ...
        <textarea type="text" placeholder="Descreva os seu problema (opcional)." value={complemento}
        onChange={ (e) => setComplemento(e.target.value)} />

        {loadInserChamados ? <button className="disableButton" type="submit" disabled={true}>Enviando...</button> : <button type="submit">{idCustomer ? 'Editar' : 'Registrar'}
        </button>}
      </form>
    </div>
